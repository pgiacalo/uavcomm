...
/**
 * A Junit test that verifies MAVLink serial communication between this client and the autopilot.
 *  - receiving MAVLink telemetry messages is tested
 *  - sending MAVLink command messages is tested
 *
 * The testing is performed by sending and receiving mavlink messages to/from the autopilot.
 * For commanding tests, messages of type msg_request_data_stream are sent to the autopilot.
 * These messages are setup to change the rates that the autopilot sends telemetry messages
 * to this client. As these telemetry messages are received, they are parsed and counted.
 * These counts are then used by to determine whether the telemetry message rates were
 * successfully changed by the commanding operations. We thereby prove whether or not
 * bi-directional communication is working.
 *
 * A successful completion of this test verifies the following:
 *  - a serial connection can be established between this client and the autopilot
 *  - mavlink command messages from this client are being received and correctly acted-upon by the autopilot
 *  - mavlink telemetry messages from the autopilot are being received and correctly interpreted by this client
 * @see https://pixhawk.ethz.ch/mavlink     [for informatoin about the "MAVLINK Common Message Set"]
 *
 * @author Philip Giacalone
 */
public class MAVLinkPortalTest implements MAVCommunicator {

    //properties object populated at runtime from the properties file
    private Properties props;

    //properties file name
    private final static String PROPERTIES_FILE_NAME = "MAVLinkPortalTest.properties";

    //the serial port used for mavlink communication
    private static MAVLinkSerialPort mavLinkSerialPort;
...
//                    System.out.println("An unhandled MAVLink message arrived, MSGID=" + mavLinkMessage.msgid);
    ...
    public void setUp() throws Exception {
        ...
        mavLinkSerialPort = new MAVLinkSerialPort(props, this);
...
//        System.out.println("mavLinkSerialPort=" + mavLinkSerialPort);
...
//        System.out.println("0) SENT, result=" + result);
    ...
    public void testMavlink() throws Exception {
        ...
        for (int streamId : STREAM_TYPES){
            ...
            MAVLinkPortalTest.initialCountMap = new HashMap<Integer, Integer>();
            MAVLinkPortalTest.finalCountMap = new HashMap<Integer, Integer>();
            ...
            Set<Integer> initialKeys = MAVLinkPortalTest.initialCountMap.keySet();
            Set<Integer> finalKeys = MAVLinkPortalTest.finalCountMap.keySet();
            System.out.println("initial message counts (msgId=count): " + MAVLinkPortalTest.initialCountMap);
            System.out.println("final message counts   (msgId=count): " + MAVLinkPortalTest.finalCountMap);
//            System.out.println("initialKeys: " + initialKeys);
//            System.out.println("finalKeys  : " + finalKeys);
            for (Integer key : initialKeys){
                if (key == HEARTBEAT_MSG_ID){
                    //skip the heartbeat message, since its rate is always 1 message/second
                    continue;
                }
//                System.out.print("msgId=" + key + ": ");
                int initialCount = MAVLinkPortalTest.initialCountMap.get(key);
                int finalCount = MAVLinkPortalTest.finalCountMap.get(key);
